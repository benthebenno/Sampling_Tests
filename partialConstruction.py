...
for row in range(y_max):
    for x_row_sum in sum_x:
        fourier_matrix[0, row] += (x_row_sum * (np.cos(2*(np.pi*(row*count)/y_max)))) - (x_row_sum *(np.sin(2*(np.pi*(2*np.pi*(row*count)/y_max)))*1j))
        count += 1
    count = 0
...
for collum in range(x_max):
    for y_row_sum in sum_y:
        fourier_matrix[collum, 0] += (y_row_sum * (np.cos(2*(np.pi*(collum*row_count)/x_max)))) - (np.sin(2*np.pi*(2*(np.pi*(collum*row_count)/x_max)))*1j)
        row_count += 1
    row_count = 0

reversedImage = ifftn(fourier_matrix)

# (x,y) = matrixSum(reversedImage)
# print(x)
# print(y)
